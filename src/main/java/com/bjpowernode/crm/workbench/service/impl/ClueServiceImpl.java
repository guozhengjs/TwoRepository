...
public class ClueServiceImpl implements ClueService {
    ...
    public boolean delete(String[] ids) {
        ...
        //查询出需要删除的备注的数量
        int count1 = clueRemarkDao.countRemarkByCids(ids);

        //删除备注，返回受到影响的条数（实际删除的数量）
        int count2 = clueRemarkDao.deleteRemarkByCids(ids);
    ...
    public List<ClueRemark> getClueRemarkById(String clueId) {
        ...
        System.out.println("打印线索备注信息2222222");
    ...
    public boolean deleteTwo(String id) {
        boolean flag = true;

        //查询出需要删除的备注的数量
        int count1 = clueRemarkDao.countRemarkByCid(id);

        //删除备注，返回受到影响的条数（实际删除的数量）
        int count2 = clueRemarkDao.deleteRemarkByCid(id);

        if(count1!=count2){
            flag = false;
        }
        //删除线索记录
        int count3 = clueDao.deleteClueTwo(id);
        if(count3!=1){
